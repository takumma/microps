...
const uint8_t EHTER_ADDR_BROADCAST[ETHER_ADDR_LEN] = {"\xff\xff\xff\xff\xff\xff"};
...
int ether_input_helper(struct net_device *dev, ether_input_func_t callback)
{
    ...
    if (memcmp(dev->addr, hdr->dst, ETHER_ADDR_LEN) != 0) {
        if (memcmp(EHTER_ADDR_BROADCAST, hdr->dst, ETHER_ADDR_LEN) != 0) {
            /* for other host */
            return -1;
        }
    }
...
void ether_setup_helper(struct net_device *dev)
{
    ...
    memcpy(dev->broadcast, EHTER_ADDR_BROADCAST, ETHER_ADDR_LEN);
}
