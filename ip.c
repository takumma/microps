...
int ip_iface_register(struct net_device *dev, struct ip_iface *iface)
{
    ...
    infof("registered: dev=%s, unicast=%s, broadcast=%s", dev->name,
...
/* NOTE: must not be call after net_run() */
int ip_protocol_register(uint8_t type, void (*handler)(const uint8_t *, size_t, ip_addr_t, ip_addr_t, struct ip_iface *))
{
    struct ip_protocol *entry;

    for (entry = protocols; entry; entry = entry->next) {
        if (entry->type == type) {
            errorf("this type already exists, type=%u", type);
            return -1;
        }
    }
...
ssize_t ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
    ...
    if (src == IP_ADDR_ANY) {
        ...
        if (!iface) {
            errorf("src iface not found, stc=%s", ip_addr_ntop(src, addr, sizeof(addr)));
            return -1;
        }
        if ((dst & iface->netmask) && (iface->unicast & iface->netmask) && dst != IP_ADDR_BROADCAST) {
            errorf("dst error, dst=%s", ip_addr_ntop(dst, addr, sizeof(addr)));
            return -1;
        }
    }
    if (NET_IFACE(iface)->dev->mtu < IP_HDR_SIZE_MIN + len) {
        errorf("too long, dev=%s, mut=%u < %zu",
