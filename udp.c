...
struct udp_pcb {
    ...
    struct queue_head queue; /* reveive queue */
...
static void udp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    debugf("%s:%d => %s:%d, len=%zu (payload=%zu)",
            ...
            ip_addr_ntop(src, addr1, sizeof(addr2)), ntoh16(hdr->dst),
            len, len - sizeof(*hdr));
    udp_dump(data, len);
    mutex_lock(&mutex);
...
ssize_t udp_output(struct ip_endpoint *src, struct ip_endpoint *dst, const uint8_t *data, size_t len)
{
    ...
    udp_dump((uint8_t *)hdr, len);

    if (ip_output(IP_PROTOCOL_UDP, (uint8_t *)hdr, len, hdr->src, hdr->dst) == -1) {
...
int udp_bind(int id, struct ip_endpoint *local)
{
    ...
    if (exist) {
        errorf("pcb already exist: id=%d, pcb=%s, local=%s",
...
ssize_t udp_sendto(int id, uint8_t *data, size_t len, struct ip_endpoint *foreign)
{
    ...
    if (!pcb->local.port) {
        ...
        if (!pcb->local.port) {
            debugf("faild to dinamic assign local port, addr=%s", ip_addr_ntop(local.addr, addr, sizeof(addr)));
            mutex_unlock(&mutex);
            return -1;
        }
    }
