...
static struct arp_cache *arp_cache_update(ip_addr_t pa, const uint8_t *ha)
{
    ...
    if (cache == NULL) {
        errorf("cache not found. pa=%u", pa);
        return NULL;
    }
...
static void arp_dump(const uint8_t *data, size_t len)
{
    ...
    fprintf(stderr, "        hln: %zu\n", message->hdr.hln);
    fprintf(stderr, "        pln: %zu\n", message->hdr.pln);
...
int arp_resolve(struct net_iface *iface, ip_addr_t pa, uint8_t *ha)
{
    ...
    cache = arp_cache_select(pa);
    if (!cache) {
        debugf("cache not found, pa=%s", ip_addr_ntop(cache->pa, addr1, sizeof(addr1)));
        mutex_unlock(&mutex);
        return ARP_RESOLVE_ERROR;
    }
    memcpy(ha, cache->ha, ETHER_ADDR_LEN);
    mutex_unlock(&mutex);
    debugf("resolved, pa=%s, ha=%s", ip_addr_ntop(cache->pa, addr1, sizeof(addr1)), ether_addr_ntop(cache->ha, addr2, sizeof(addr2)));
...
static void arp_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    if (iface && ((struct ip_iface *)iface)->unicast == tpa) {
        ...
        if (msg->hdr.op == ARP_OP_REQUEST) {
            arp_reply(iface, msg->tha, msg->tpa, msg->sha);
        }
    }
}
