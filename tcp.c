...
static void tcp_retransmit_queue_cleanup(struct tcp_pcb *pcb)
{
    ...
    while (1) {
        ...
        debugf("remove, seq=%um flags=%s, len=%u", entry->seq, tcp_flg_ntoa(entry->flg), entry->len);
        memory_free(entry);
    }
    return;
}
...
static void tcp_segment_arrives(struct tcp_segment_info *seg, uint8_t flags, uint8_t *data, size_t len, struct ip_endpoint *local, struct ip_endpoint *foreign)
{
    ...
    case TCP_PCB_STATE_LISTEN:
        ...
        if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
            ...
            /* but processing of SYN and ACK sould not be repeated              */
            return;
        }
        /*
    ...
    case TCP_PCB_STATE_SYN_SENT:
        ...
        /*
         * 2nd check for an RST bit
         */
        
        /*
         * 3rd check security and procedence (ignore)
         */
        ...
        if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
            ...
            if (pcb->snd.una > pcb->iss) {
                ...
                pcb->state = TCP_PCB_STATE_SYN_RECEIVED;
                tcp_output(pcb, TCP_FLG_SYN | TCP_FLG_ACK, NULL, 0);
                /* ignore: If the are other controls or text in the segment, queue them for processing after the ESTABLISHED state has been reached */
                return;
            }
    ...
    case TCP_PCB_STATE_LAST_ACK:
         ...
         * segment that begins at RCV.NXT and does not exceed that window.
         * One could tailor actual segments to fit this assumption by
         * trimming off any portions that lie outside the window (including
         * SYN and FIN), and only procesing further if the segment then
...
static void tcp_input(const uint8_t *data, size_t len, ip_addr_t src,ip_addr_t dst, struct ip_iface *iface)
{
    ...
    if (src == IP_ADDR_BROADCAST || src == iface->broadcast ||  dst == IP_ADDR_BROADCAST || dst == iface->broadcast) {
        errorf("src or dst is same to broadcast, src=%u, dst=%u, broadcast=%u",
            ntoh16(hdr->src), ntoh16(hdr->dst), ntoh16(iface->broadcast));
        return;
    }
...
int tcp_init(void)
{
    ...
    if (net_timer_regiseter(interval, tcp_timer) == -1) {
        errorf("net_timer_regiseter() failure");
        return -1;
    }
    return 0;
}
