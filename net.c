...
int net_timer_regiseter(struct timeval interval, void (*handler)(void))
{
...
int net_device_register(struct net_device *dev)
{
    ...
    dev->next = devices;
    devices = dev;
    infof("registered d, dev=%s, type=0x%04x", dev->name, dev->type);
    return 0;
}
...
int net_protocol_register(uint16_t type, void (*handler)(const uint8_t *data, size_t len, struct net_device *dev))
{
    ...
    for (proto = protocols; proto; proto = proto->next) {
        if (type == proto->type) {
            errorf("already registed, type=0x%04x", type);
            return -1;
        }
    }
...
/* NOTE: must not be call after net_run() */
int net_event_subscribe(void (*handler)(void *), void *arg)
{
